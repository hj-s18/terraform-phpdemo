...
mysql -u${db_username} -p${db_password} -h${db_url} ${db_name} < /phpdemo/create_db_webtest.sql     # MySQL 서버에 연결하고, SQL 파일을 실행하여 데이터베이스 초기화 작업 수행하거나 테이블 등을 생성함 (테이블 생성하는 코드 들어있음)
