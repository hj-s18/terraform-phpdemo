...
git clone -b create-table https://github.com/hj-s18/terraform-practice.git                                     # git 저장소 클론(복제) : 저장소가 현재 디렉토리 안에 phpdemo 폴더로 복제됨 (create-table 브랜치에 포함된 파일만 가져옴)
mysql -u${db_username} -p${db_password} -h${db_url} ${db_name} < /terraform-practice/create_table_items.sql    # MySQL 서버에 연결하고, SQL 파일을 실행하여 데이터베이스 초기화 작업 수행하거나 테이블 등을 생성함 (테이블 생성하는 코드 들어있음)
